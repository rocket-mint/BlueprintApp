...
export function EditButton({
  ...
  return (
    <button
      ...
      title={`Edit ${type}`}
      className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-brand-cyan-500/10 text-brand-cyan-500 transition-colors hover:bg-brand-cyan-500 hover:text-white"
    >
  ...
  return (
    <>
      <button
        ...
        title={`Delete ${type}`}
        className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-semantic-error/10 text-semantic-error transition-colors hover:bg-semantic-error hover:text-white"
      >
        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
